 ...
 */
export default class EnemySpawner<EnemyType extends IEnemy> extends Phaser.GameObjects
  .GameObject {
  /**
  ...
  protected getNextSpawnPosition(): Math.Vector2 {
    ...
    const x = axis === 0 ? Math.Between(0, 1280) : axis === -1 ? 0 : 1280;
  ...
  protected setTargetAndMotion(enemy: EnemyType): void {
    ...
    enemy.setAcceleration(
      Math.Between(this.config.accellMin, this.config.accellMax)
    );
  }
}
